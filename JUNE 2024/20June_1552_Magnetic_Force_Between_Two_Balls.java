...
//Binary Search
//same as little-> aggressive cows 

class Solution {
    // TC:O(log(maxForce*N)) SC:O(1)
    public int maxDistance(int[] pos, int m) {
        Arrays.sort(pos);
        // minForce=1

        // maxForce=diff betweem max and min element in pos array
        int pans = -1;
        int minForce = 1;
        int maxForce = pos[pos.length - 1] - pos[0];
        int lo = minForce, hi = maxForce;
        while (lo <= hi) { // O(logN)
            int mid = lo + (hi - lo) / 2;

            if (isPossible(pos, mid, m)) { // TC:O(maxForce)
                pans = mid;
                lo = mid + 1;
            } else {
                hi = mid - 1;
    ...
    public boolean isPossible(int[] pos, int force, int m) {
        int prevBalls = pos[0];
        int cntBalls = 1;

        for (int i = 1; i < pos.length; i++) {
            int currBalls = pos[i];
            if (currBalls - prevBalls >= force) {
                cntBalls++;
                prevBalls = currBalls;
                if (cntBalls == m)
                    return true;
