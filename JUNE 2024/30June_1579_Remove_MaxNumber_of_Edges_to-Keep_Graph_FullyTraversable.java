class Solution {
    public int maxNumEdgesToRemove(int n, int[][] edges) {
        ...
        // bcz on type 3 edges both can walk
        ...
        for (int i = 0; i < n + 1; i++) {// every node is pointing to itself, at first no connection is considered all
                                         // sets are independent(no dependency)
            parentAlice[i] = i;
            parentBob[i] = i;
        }

        // number of merged unique node for Alice and Bob that are required to maintain
        // the connectedness of Alice and Bob graph nodes//intialised with one because
        // merging happens in pair
        int mergeAlice = 1;
        int mergeBob = 1;

        // number of cyclic or the non dependent node, that are not required for the
        // connectedness of Alice and Bob nodes
        int removeEdge = 0;

        for (int[] edge : edges) {
            int cat = edge[0];// category of edge 1)edge Alice can only access 2)edge Bob can only access
                              // 3)edge both Alice and Bob can access
            int u = edge[1];
            int v = edge[2];
            ...
            if (cat == 3) {// edge both Alice and Bob an access
                ...
                if (tempAlice == false && tempBob == false)// retundant or the cyclic non-dependent edge//both Alice and
                                                           // Bob don't rquire it connection is already there between
                                                           // these pair of nodes
                    removeEdge += 1;
                ...
                if (tempBob == true)
                    mergeBob += 1;
                else// no merging of set is done, that means that this edge is not required because
                    // it will form cycle or the dependency
                    removeEdge += 1;
    ...
    public int find(int x, int[] parent) {
        ...
        parent[x] = temp;// Path Compression//child pointing to the absolute root or the leader of the
                         // set, while backtracking
